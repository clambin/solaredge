...
// GetSiteDataPeriod returns the energy production start and end dates of the site.
//
// Note: unlike the example in the specs, this only returns the date, not the time of day.
func (c *Client) GetSiteDataPeriod(ctx context.Context, id int) (GetSiteDataPeriodResponse, error) {
	return call[GetSiteDataPeriodResponse](ctx, c, makePath("/site/{siteId}/dataPeriod", id), nil)
}

type GetSiteDataPeriodResponse struct {
	DataPeriod SiteDataPeriod `json:"dataPeriod"`
}

type SiteDataPeriod struct {
	StartDate Date `json:"startDate"`
	EndDate   Date `json:"endDate"`
}

// GetSiteEnergy return the site energy measurements.
//
...
func (c *Client) GetSiteEnergy(ctx context.Context, id int, timeUnit TimeUnit, startDate time.Time, endDate time.Time) (GetSiteEnergyResponse, error) {
	args := url.Values{
	...
	return call[GetSiteEnergyResponse](ctx, c, makePath("/site/{siteId}/energy", id), args)
}

type GetSiteEnergyResponse struct {
	Energy SiteEnergy `json:"energy"`
}
...
// GetSiteEnergyForTimeFrame returns the site total energy produced for a given period.
//
// Notes:
//   - This API only returns on-grid energy for the requested period. In sites with storage/backup, this may mean that results can differ from what appears in the Site Dashboard. Use the regular Site Energy API to obtain results that match the Site Dashboard calculation.
//   - The period between end and start must not exceed one year.
func (c *Client) GetSiteEnergyForTimeFrame(ctx context.Context, id int, startDate, endDate time.Time) (GetSiteEnergyForTimeframeResponse, error) {
	args := url.Values{
		"startDate": []string{startDate.Format(time.DateOnly)},
		"endDate":   []string{endDate.Format(time.DateOnly)},
	}
	return call[GetSiteEnergyForTimeframeResponse](ctx, c, makePath("/site/{siteId}/timeFrameEnergy", id), args)
}

type GetSiteEnergyForTimeframeResponse struct {
...
type SiteEnergyForTimeframe struct {
	...
	MeasuredBy          string             `json:"measuredBy"`
	StartLifetimeEnergy SiteLifetimeEnergy `json:"startLifetimeEnergy"`
	EndLifetimeEnergy   SiteLifetimeEnergy `json:"endLifetimeEnergy"`
}

type SiteLifetimeEnergy struct {
...
// GetSitePower returns the site power measurements in 15 minute intervals.
//
// Note: This API is limited to a one-month period. If the provided time range exceeds one month, an APIError is returned.
func (c *Client) GetSitePower(ctx context.Context, id int, startTime, endTime time.Time) (GetSitePowerResponse, error) {
	args := url.Values{
		"startTime": []string{startTime.Format(timeFormat)},
		"endTime":   []string{endTime.Format(timeFormat)},
	}
	return call[GetSitePowerResponse](ctx, c, makePath("/site/{siteId}/power", id), args)
}

type GetSitePowerResponse struct {
...
// GetSitePowerOverview returns the energy produced at the site for its entire lifetime, current year, month and day (in Wh) and current power (in W).
func (c *Client) GetSitePowerOverview(ctx context.Context, id int) (GetSitePowerOverviewResponse, error) {
	return call[GetSitePowerOverviewResponse](ctx, c, makePath("/site/{siteId}/overview", id), nil)
}

type GetSitePowerOverviewResponse struct {
...
// GetSitePowerDetails returns site power measurements from meters such as consumption, export (feed-in), import (purchase), etc.
//
// Note: This API is limited to one-month period. If the provided time range exceeds one month, an APIError is returned.
func (c *Client) GetSitePowerDetails(ctx context.Context, id int, start, end time.Time) (GetSitePowerDetailsResponse, error) {
	args := url.Values{
		"startTime": []string{start.Format(timeFormat)},
		"endTime":   []string{end.Format(timeFormat)},
	}
	return call[GetSitePowerDetailsResponse](ctx, c, makePath("/site/{siteId}/powerDetails", id), args)
}

type GetSitePowerDetailsResponse struct {
...
// GetSiteEnergyDetails returns site energy measurements from meters such as consumption, export (feed-in), import (purchase), etc.
...
func (c *Client) GetSiteEnergyDetails(ctx context.Context, id int, timeUnit TimeUnit, startTime, endTime time.Time) (GetSiteEnergyDetailsResponse, error) {
	args := url.Values{
		"startTime": []string{startTime.Format(timeFormat)},
		"endTime":   []string{endTime.Format(timeFormat)},
		"timeUnit":  []string{string(timeUnit)},
	}
	return call[GetSiteEnergyDetailsResponse](ctx, c, makePath("/site/{siteId}/energyDetails", id), args)
}

type GetSiteEnergyDetailsResponse struct {
	EnergyDetails SiteEnergyDetails `json:"energyDetails"`
}

// SiteEnergyDetails contains site energy measurements from meters such as consumption, export (feed-in), import (purchase), etc.
...
// GetSitePowerFlow returns the current power flow between all elements of the site including PV array, storage (battery), loads (consumption) and grid.
//
// TODO: test this during daytime
func (c *Client) GetSitePowerFlow(ctx context.Context, id int) (GetSitePowerFlowResponse, error) {
	return call[GetSitePowerFlowResponse](ctx, c, makePath("/site/{siteId}/currentPowerFlow", id), nil)
}

type GetSitePowerFlowResponse struct {
	CurrentPowerFlow CurrentPowerFlow `json:"siteCurrentPowerFlow"`
}
...
// GetSiteStorageData returns detailed information from batteries installed at the active site.
//
// This API is limited to a one-week period.
func (c *Client) GetSiteStorageData(ctx context.Context, id int, startTime, endTime time.Time) (GetSiteStorageDataResponse, error) {
	args := url.Values{
		"startTime": []string{startTime.Format(timeFormat)},
		"endTime":   []string{endTime.Format(timeFormat)},
	}
	return call[GetSiteStorageDataResponse](ctx, c, makePath("/site/{siteId}/storageData", id), args)
}

type GetSiteStorageDataResponse struct {
	StorageData struct {
...
// GetSiteEnvBenefits returns all environmental benefits based on site energy production: gas emissions saved, equivalent trees planted and light bulbs powered for a day.
func (c *Client) GetSiteEnvBenefits(ctx context.Context, id int) (SiteEnvBenefitsResponse, error) {
	return call[SiteEnvBenefitsResponse](ctx, c, makePath("/site/{siteId}/envBenefits", id), nil)
}

type SiteEnvBenefitsResponse struct {
